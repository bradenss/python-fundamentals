...
# and uses square brackets to define and collected
...
# using len o get the length of a list
def simple_length():
...
def simple_insert():
    ...
    # Friday gets moved to position 3
    for value in mixed:
        print(value)
...
# Removing items from a list be remove(), pop(), del and clear()
...
# clear method will empty a list, but will not remove the list.
def simple_clear():
    mixed.clear()
    mixed.append('Python')
    for mix in mixed:
...
# the key is unique and con not be duplicated making immutable
...
# nested dictionaries refer to having a dictionary with a dictionary
